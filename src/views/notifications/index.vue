  ...
  div
    ...
    .blink-50(v-else)
      ul
        ...
        li.list-item.mb-0.loading-2
          .container
            .list-title
            i.fas.fa-spinner.fa-pulse.fa-2x

        li.list-item.mb-0.loading-3
          .container
            .list-title
            i.fas.fa-spinner.fa-pulse.fa-2x

        li.list-item.mb-0.loading-4
          .container
            .list-title
            i.fas.fa-spinner.fa-pulse.fa-2x
  ...
  export default {
    ...
    methods: {
      parseNotificationPath(notification) {
        ...
        if((/^(bidding)/i).test(action)) return this.biddingsRoute(notification)
      ...
      biddingsRoute(notification) {
        return { name: 'bidding', params: { id: notification.notifiable_id } }
      },

      invitesRoute(notification) {
        let args = notification.args

        return { name: 'invites', params: { bidding_id: args.bidding_id } }
      },

      lotProposalsRoute(notification) {
        let args = notification.args
      ...
      clickedNotification(notification) {
        ...
        this.$http.patch('/cooperative/notifications/' + notification.id + '/mark_as_read')
      ...
      fetch() {
        ...
        return this.$http.get('/cooperative/notifications', { params })
          .then((response) => {
    ...
    created: function () {
      ...
      this.$emit('navbarTitleChanged', this.$t('navbar.notification'))
