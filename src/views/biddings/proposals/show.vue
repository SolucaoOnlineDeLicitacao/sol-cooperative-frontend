  ...
  .mt-2
    ...
    .container(v-else)
      ...
      .card.mt-2.mb-2(v-for="(lot_proposal, index) in lot_proposals")
        ...
        ul.mb-0
          template(v-for="(lot_group_item_lot_proposal, index) in lot_proposal.lot_group_item_lot_proposals")
    ...
    accept-proposal-overlay(:showOverlay="showAcceptOverlay", :item="acceptOverlayItem", @closeOverlay="showAcceptOverlay = false", @success="successAccept", routeName="proposals")

    refuse-proposal-overlay(:showOverlay="showRefuseOverlay", :item="refuseOverlayItem", @closeOverlay="showRefuseOverlay = false", @success="successRefuse", routeName="proposals")

</template>

<script>
  import AcceptProposalOverlay from '../lots/proposals/_index/accept-proposal-overlay'
  import RefuseProposalOverlay from '../lots/proposals/_index/refuse-proposal-overlay'
  import ProposalAlerts from '../lots/proposals/_show/proposal-alerts'
  import ProposalListItem from '../lots/proposals/_show/proposal-list-item'
    ...
    methods: {
      fetch() {
        ...
        return this.$http.get('/cooperative/biddings/' + this.biddingId)
          ...
          .then((response) => {
            this.$http.get('/cooperative/biddings/' + this.biddingId + '/proposals/' + params.id)
              .then((response) => {
