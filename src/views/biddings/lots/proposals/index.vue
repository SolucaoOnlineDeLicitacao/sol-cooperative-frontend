      ...
      .card.slim
        ...
        ul.mb-0(v-else-if="proposalsCount")
          template(v-for="(proposal, index) in proposals")
            proposal-item(:proposal="proposal", :bidding="bidding", :index="index", @successAccept="fetch", @successRefuse="fetch")

        ul.mb-0(v-else-if="!seeProposals")
          li.list-item.pb-0
            .container.mt-2.mb-2
              | {{ $t('.cant_see') }}
...
<script>
  import ProposalItem from './_index/proposal-item'
